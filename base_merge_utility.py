...
print("Найдены файлы главной базы:", file_name_main_base_found)
...
if len(file_name_main_base_found) == 1:
    ...
    for file in file_name_main_base_found:
        if file.startswith("~$"):
...
# ----------------------------------------------------------------
# 2=FILES - NEW PRICE
file_name_price_new_found = glob.glob("*Прайс*.xlsx")
print("Найдены файлы новых цен:", file_name_price_new_found)

for file in file_name_price_new_found:
    if file.startswith("~$"):
        print("ОШИБКА: файл главной базы открыт! закройте его и перезапустите приложение!", file=sys.stderr)
        input("Нажмите Enter для выхода")
        sys.exit()
